...
页面大小4k，逻辑地址16进制 1D16H  十进制     求 物理地址？     
...
磁盘 字长32位 4字节B  物理块 单个4M   1024 *1024  物理块编号0/1/2   16384 号

位示图   磁盘容量 除以 字长 * 块大小  等于字

不产生死锁    先分配 后申请   资源 出方向是 分配  进方向是申请

化简过程是：  剩余资源未分配  -》 非阻塞节点

pv操作  打印机 初始值2 个信号量    p表示申请一个资源  v表示释放一个资源   互斥信号量 资源唯一   同步信号量 通知

互斥信号量为1 

磁盘读取时间 读10  5   处理2  10 

100个磁道  6s   10个磁道  120s  

索引：  求二级索引最大长度 K字节  是一级索引的平方    块大小/块所占字节      1K /3B   *1024 = B 块好  是以及索引的长度 平方
